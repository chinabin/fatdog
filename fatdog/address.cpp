...
namespace fatdog
{
                                        ...
                                        int family)
    {
        ...
        {
            FATDOG_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                                          " err="
        ...
        {
            FATDOG_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
            freeifaddrs(results);
            return false;
        }
        freeifaddrs(results);
    ...
    IPAddress::ptr IPAddress::Create(const char *address, uint16_t port)
    {
        ...
        if (error)
        {
            FATDOG_LOG_ERROR(g_logger) << "IPAddress::Create(" << address
                                       << ", " << port << ") error=" << error << " errstr=" << gai_strerror(error);
            return nullptr;
        }
    ...
    IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port)
    {
        ...
        if (result <= 0)
        {
            FATDOG_LOG_ERROR(g_logger) << "IPv4Address::Create(" << address << ", "
